...
import numpy as np
import torch
from transformers import BertConfig, BertTokenizer, BertForMaskedLM, AutoTokenizer, AutoConfig, AutoModelForMaskedLM
...
class BAEAttacker(ClassificationAttacker):
    ...
    def __init__(self, 
        ...
        self.tokenizer_mlm = AutoTokenizer.from_pretrained(mlm_path, do_lower_case=True)
        ...
        self.mlm_model = AutoModelForMaskedLM.from_pretrained(mlm_path, config=config_atk).to(self.device)
        self.k = k
    ...
    def _get_masked(self, words):
        ...
        for i in range(len_text - 1):
            masked_words.append(words[0:i] + [self.tokenizer_mlm.unk_token] + words[i + 1:])
        # list of words
        return masked_words
    ...
    def _get_masked_insert(self, words):
        ...
        masked_words = []
        for i in range(len_text - 1):
            masked_words.append(words[0:i + 1] + [self.tokenizer_mlm.unk_token] + words[i + 1:])
        # list of words
        return masked_words
