...
class AttackEval:
    ...
    def __iter_metrics(self, iterable_result):
        for data, result in iterable_result:
            ...
            if adversarial_sample == '': adversarial_sample = None
            ret = {
                "data": data,
    ...
    def eval(self, dataset: Iterable[Dict[str, Any]], total_len : Optional[int] = None, visualize : bool = False, progress_bar : bool = False, num_workers : int = 0, chunk_size : Optional[int] = None):
        ...
        for d in metrics_l:
            for key, value in d.items():
                metric_d.setdefault(key, []).append(value)

        for k,v in metric_d.items(): 
            if k not in ['Succeed', 'Query Exceeded']:
                summary[f'{k} (Median)'] = np.median(v)

        if visualize:
            result_visualizer(summary, sys.stdout.write)
        results_df = self.results2df()
    ...
    def results2df(self): 
        ...
        df.drop(columns=['data.x', 'data.y', 'data.input_ids', 'data.token_type_ids', 'data.attention_mask', 'metrics.Succeed'], 
                inplace=True, errors='ignore')
        df.columns = df.columns.str.replace('metrics.', '')
        df.columns = df.columns.str.replace('data.', '')
        df = df.query("success == True")
        return df